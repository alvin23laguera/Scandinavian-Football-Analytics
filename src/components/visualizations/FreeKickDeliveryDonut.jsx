...
const FreeKickDeliveryDonut = ({ data = [], teamName = "Tromsø IL" }) => {
    ...
    const totalFreeKicks = processedData.length;
    const totalCompleted = processedData.filter(d => d.outcome === 'completed').length;
    const completedPerc = totalFreeKicks > 0 ? Math.round((totalCompleted / totalFreeKicks) * 100) : 0;
    ...
    const generateArcs = () => {
        let currentAngle = 0;
        const gap = totalFreeKicks > 0 ? 3 : 0; // 3 degree gap between segments
        ...
        Object.keys(stats).forEach(key => {
            ...
            const segmentAngle = (stat.total / totalFreeKicks) * availableAngle;
                            ...
                            <g key={arc.key}>
                                ...
                                {(() => {
                                    ...
                                    const value = viewMode === 'percent' 
                                        ? `${Math.round((arc.stat.total / totalFreeKicks) * 100)}%`
                                        : arc.stat.total;
...
export default FreeKickDeliveryDonut;
